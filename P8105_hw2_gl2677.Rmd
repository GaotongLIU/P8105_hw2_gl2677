...
The key variables for furthre research are ``r colnames(pols_snp_unemp[3:11])``, and I use `year` and `month` as the key to tidy the dataset. The range of year is from `r min(pull(pols_snp_unemp, year))` to `r max(pull(pols_snp_unemp, year))`.
