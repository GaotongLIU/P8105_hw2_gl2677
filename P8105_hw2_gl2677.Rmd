...
The key variables are `r colnames(pols_month[3:7])`. The range of year is from `r min(pull(pols_month, year))` to `r max(pull(pols_month, year))`.
...
There are `r nrow(pols_snp_unemp)` observations and `r ncol(pols_snp_unemp[3:11])` variables in `pols_snp_unemp` **resulting dataset**. 
