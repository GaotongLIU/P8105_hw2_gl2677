...
litter, litter type and homes powered are most important. The key
...
sports_balls, homes_powered`. The median number of sports balls in a
...
and year. The key variables are `total`. The total precipitation in 2018
was 23.5.
...
variables are `gov_gop, sen_gop, rep_gop, gov_dem, sen_dem, rep_dem,
president`. The range of year is from 1947 to 2015.
...
given time. The key variables are `close`. The range of year is from
1950 to 2015.
...
**resulting dataset**. The key variables are `gov_gop, sen_gop, rep_gop,
gov_dem, sen_dem, rep_dem, president, close, percentage`. The range of
